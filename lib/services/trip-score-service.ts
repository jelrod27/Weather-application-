 ...
 * Trip Score Service
 *
 * Composes drive/fly trip misery scores for /api/travel/trip-score:
 *  - drive: nearest US interstate corridor, Open-Meteo weather per waypoint
 *  - fly: hub airports via METAR, en-route SIGMET/AIRMET near the midpoint
 *
 * Uses combineMiseryScores so a single brutal segment dominates the trip score.
 */
...
const TRIP_SCORE_CACHE_CONTROL = 'public, s-maxage=600, stale-while-revalidate=300';
...
export async function computeDriveTripScore(
  ...
  return NextResponse.json(
    ...
    {
      headers: {
        'Cache-Control': TRIP_SCORE_CACHE_CONTROL,
      },
    },
  );
}

export async function computeFlyTripScore(
  origin: ResolvedEndpoint,
  destination: ResolvedEndpoint,
): Promise<NextResponse> {
