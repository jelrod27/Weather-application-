...
word_count: 1066
fact_check_claims: 41
fact_check_unsupported: 10
sources:
  - label: NWS Glossary — Stratus
    url: https://forecast.weather.gov/glossary.php?word=stratus
  - label: NOAA JetStream — How Clouds Form
    url: https://www.noaa.gov/jetstream/clouds/how-clouds-form
  - label: NOAA JetStream — Ten Basic Clouds
  ...
  - label: NWS Glossary — Cirrostratus
    url: https://forecast.weather.gov/glossary.php?word=cirrostratus
  - label: NWS Glossary — Stratocumulus
    url: https://forecast.weather.gov/glossary.php?word=stratocumulus
  - label: NOAA JetStream — Four Core Types of Clouds
    url: https://www.noaa.gov/jetstream/clouds/four-core-types-of-clouds
fact_check_flagged:
  - Stratus typically forms when an entire layer of air is cooled uniformly over hours until the whole layer reaches its dew point simultaneously.
  - Because cooling is spread horizontally, stratus saturation occurs at nearly the same level everywhere, producing a sharply defined base with no internal texture.
  - A cumulus tower takes roughly twenty minutes to build.
  - Morning warming of the lowest few hundred feet evaporates the bottom of a fog layer, causing it to lift into a low overcast.
  - The marine layer forms when air over cold water is chilled from below until it saturates, and onshore flow pushes it inland.
  - Stratus decks are typically a few hundred to a couple of thousand feet thick, allowing the sun to burn through by early afternoon.
  - Rain-sized drops grow by repeated recirculation and collision within updrafts.
  - Stratus lacks updrafts, so its droplets stay small and many evaporate before reaching the ground.
  - Stratus is the cloud genus most likely to have its base at or near the surface.
  - A lifted stratus deck can lower back to the surface as the layer cools again.
diagrams:
  - id: cloud-altitude-plot
    insertAfter: burns through by early afternoon
---
